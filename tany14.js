(async () => {
  ...
  alert("⏳ Waiting for admin approval... (up to 2 minutes)");

  const checkAccess = async () => {
    try {
      const response = await fetch(`http://localhost:3000/check-access?email=${encodeURIComponent(email)}`);
      const script = await response.text();

      if (response.ok && !script.startsWith('//')) {
        eval(script); // ✅ Access approved, run code
        return true;
      } else {
        console.log(`⏳ Still waiting... (${elapsed}s):`, script);
        return false;
      }
    } catch (e) {
  ...
  const pollUntilApproved = async () => {
    const approved = await checkAccess();

    if (approved) return;

    elapsed += pollInterval / 1000;
    if (elapsed >= maxWaitSeconds) {
      alert("❌ Timeout: Access not approved within 2 minutes.");
      return;
    }

    setTimeout(pollUntilApproved, pollInterval); // ⏱️ Wait & retry
  };

  pollUntilApproved(); // 🟢 Start polling
})();
