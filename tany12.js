(async () => {
  ...
  console.log(email);
  const url = `http://localhost:3000/check-access?email=${encodeURIComponent(email)}`;
  console.log(url)
  ...
  try {
    ...
    console.log("📄 Response text:", script);

    if (response.ok) {
      if (!script.startsWith('//')) {
        eval(script); // Only run if it's actual script
      } else {
        alert(script); // Server sent a status message
      }
    } else {
      alert(script); // Likely: pending or rejected
    }
  } catch (e) {
    console.error("❌ Fetch failed:", e);
    alert("❌ Failed to contact access server. Is the backend running?");
  }
})();
(async () => {
