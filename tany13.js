(async () => {
  ...
  const checkAccess = async () => {
    try {
      ...
      if (response.ok && !script.startsWith('//')) {
        eval(script); // ✅ Approved & valid script
        return true;  // Stop the loop
      } else {
        console.log("⏳ Still pending or rejected:", script);
        return false;
      }
    } catch (e) {
  ...
  const waitForApproval = setInterval(async () => {
    const approved = await checkAccess();
    elapsed += interval / 1000;

    if (approved) {
      clearInterval(waitForApproval);
    } else if (elapsed >= maxWait) {
      clearInterval(waitForApproval);
      alert("❌ Timeout: Admin did not approve access within 2 minutes.");
    }
  }, interval);
})();
(async () => {
