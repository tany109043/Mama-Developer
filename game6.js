...
`;
    ...
    matchingGameBtn.onclick = async () => {
        // Extract course heading/title
        const courseTitle = document.querySelector('h1')?.innerText || 'Untitled Course';

        // Show loading UI
        matchingGameOverlay.innerHTML = `
      <div class="game-container" style="position:relative;min-height:220px;display:flex;align-items:center;justify-content:center;">
        <div style="width:100%;text-align:center;font-size:18px;padding:40px 0;">⏳ Generating matching questions for<br><b>${courseTitle}</b>…</div>
      </div>
    `;
        matchingGameOverlay.style.display = "flex";

        // AI prompt for related term/definition pairs
        const matchPrompt = `
...
`;
        ...
        try {
            ...
            matchingGameOverlay.innerHTML = `
          <div class="game-container" style="position:relative;min-height:220px;display:flex;align-items:center;justify-content:center;">
            <div style="width:100%;text-align:center;font-size:18px;padding:40px 0;color:red;">
              ❌ Failed to generate questions for<br><b>${courseTitle}</b>.<br>Please try again.
            </div>
