...
`;
  ...
  // === Dynamic Matching Game – Udemy Modules x Cohere ===
// Purpose: show a term–definition drag‑and‑drop game that auto‑builds
//   pairs from the current Udemy course page using Cohere's LLM.
// Usage: include this file in your bookmarklet script bundle.
// ⚠️  Insert your Cohere API key in COHERE_API_KEY before use.
...
(function () {
  ...
  function addTokens(delta) {
    ...
    console.log(`[Matching‑Game] Token change ${delta >= 0 ? "+" : ""}${delta}. Total: ${next}`);
  }

  async function fetchPairsFromCohere(modules) {
    // Build prompt instructing Cohere to return JSON array of {term, definition}
    const prompt = `You are a concise assistant that creates quiz content.\n` +
                   `Given these Udemy course module titles, produce up to ${MAX_PAIRS} ` +
                   `JSON array items with \n  • \"term\": a keyword or concept from the module, \n  • \"definition\": a short (≤7 words) explanation.\n` +
    ...
    try {
      const res = await fetch("https://api.cohere.ai/v1/generate", {
        ...
        body: JSON.stringify({
          model: "command-r-plus", // choose your plan's model
          max_tokens: 120,
          temperature: 0.4,
          truncate: "END",
      ...
      if (Array.isArray(parsed) && parsed.length) return parsed.slice(0, MAX_PAIRS);
      throw new Error("Could not parse pairs");
    } catch (err) {
      console.warn("[Matching‑Game] Cohere error", err);
      return [];
    }
  }

  function getUdemyModuleTitles() {
    // Udemy uses [data-purpose="course-item-title"] or similar in the DOM.
    // We try a few selectors – adjust if Udemy updates its markup.
    const titles = new Set();
    document.querySelectorAll('[data-purpose="course-item-title"], .lecture-title-text, [data-purpose="curriculum-section-title"]')
      .forEach(el => {
  ...
  /* --------------------------- GAME INITIALISER --------------------------- */
  async function launchMatchingGame() {
    const headerBar = document.body; // adjust if you want another anchor

    // 1. Add main launcher button if not present
    if (!document.getElementById("matchingGameLauncher")) {
      const matchingGameBtn = document.createElement("button");
      matchingGameBtn.id = "matchingGameLauncher";
      matchingGameBtn.textContent = "🎮 Matching Game";
      matchingGameBtn.style.cssText = "padding:6px 14px;background:#009688;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:13px;z-index:10002;position:fixed;bottom:24px;right:24px;box-shadow:0 3px 12px rgba(0,0,0,0.2);";
      headerBar.appendChild(matchingGameBtn);
      matchingGameBtn.onclick = showGame;
    }
  }

  /* -------------------------- GAME RENDER / LOGIC ------------------------- */
  async function showGame() {
    let overlay = document.getElementById("matchingGameOverlay");
    if (!overlay) {
      overlay = document.createElement("div");
    ...
    overlay.innerHTML = "<div class=\"game-container\">Loading…</div>";
    overlay.querySelector(".game-container").style.cssText = "background:#fff;padding:32px 24px;border-radius:16px;font-family:sans-serif;min-width:260px;display:flex;flex-direction:column;align-items:center;gap:12px;box-shadow:0 6px 24px rgba(0,0,0,0.15);";
    overlay.onclick = e => { if (e.target === overlay) closeGame(); };

    // Fetch pairs (Udemy + Cohere → pairs)
    const modules = getUdemyModuleTitles();
    let pairs = await fetchPairsFromCohere(modules);
    if (!pairs.length) {
      // Fallback default pairs
      pairs = [
        { term: "ObjectId", definition: "MongoDB unique document ID" },
        { term: "insert_one", definition: "Insert one document" },
        { term: "Pandas", definition: "Python data analysis lib" },
        { term: "update_one", definition: "Update one document" },
        { term: "Dictionary", definition: "Python key‑value type" }
      ];
    }
  ...
  function buildGameUI(overlay, pairs) {
    ...
    <div class="game-container">
      ...
      <div id="scoreBoard">⏱ <span id="timer">${TIME_LIMIT}</span>s | ✅ Score: <span id="score">0</span>/${pairs.length}</div>
      <div id="game" style="display:flex;flex-wrap:wrap;gap:12px;justify-content:center;"></div>
      <p style="font-size:12px;color:#555;text-align:center;margin:8px 0 0;">Drag the terms onto their definitions.<br>Wrong drops disable an option.</p>
    </div>`;

    const container = overlay.querySelector(".game-container");
    const gameDiv = container.querySelector("#game");

    // styling via a <style> tag
    if (!document.getElementById("matchingGameStyles")) {
      const styleEl = document.createElement("style");
      styleEl.id = "matchingGameStyles";
      styleEl.textContent = `
      .game-container{background:linear-gradient(135deg,#e3f0ff 0%,#f9f6ff 100%);border:1.5px solid #b6c7e6;border-radius:16px;box-shadow:0 6px 24px rgba(80,120,200,0.10);position:relative;min-width:270px;max-width:400px}
      ...
      #scoreBoard{margin:10px 0;font-size:14px;color:#333;background:#fff;border-radius:8px;padding:6px 12px;box-shadow:0 2px 8px rgba(80,120,200,.06);display:inline-block}
      ...
      .close-btn{position:absolute;top:10px;right:18px;font-size:22px;font-weight:bold;color:#888;background:#fff;border:1.5px solid #b6c7e6;border-radius:50%;width:32px;height:32px;line-height:28px;text-align:center;cursor:pointer;box-shadow:0 2px 8px rgba(80,120,200,.10);transition:background .2s,color .2s;z-index:10}
      .close-btn:hover{background:#f44336;color:#fff;border-color:#f44336}`;
      document.head.appendChild(styleEl);
    }

    // Build draggable terms & drop boxes
    ...
    shuffledTerms.forEach(p => {
      ...
      div.id = `term-${p.term.replace(/\s+/g, "-")}`;
      div.ondragstart = e => {
        if (div.classList.contains("disabled")) { e.preventDefault(); }
    ...
    [...pairs].sort(() => Math.random() - 0.5).forEach(({ term, definition }) => {
      ...
      box.ondrop = e => {
        ...
        if (dragged === term) {
          ...
          const draggedEl = document.getElementById(`term-${dragged.replace(/\s+/g, "-")}`);
          draggedEl && draggedEl.remove();
          checkGameEnd();
        } else {
    ...
    container.querySelector(".close-btn").onclick = closeGame;

    function checkGameEnd() {
      const remaining = [...overlay.querySelectorAll('.term')].filter(el => !el.classList.contains('disabled'));
      if (matched === pairs.length || remaining.length === 0) endGame();
    }
